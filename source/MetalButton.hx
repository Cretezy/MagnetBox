 ...
 */
class MetalButton extends FlxTypedMetaButton<FlxText> {
	private static inline var MOBILE_SCALE = 1.5;

	#if js
	public static var DEVICE_SCALE = FlxG.html5.onMobile ? MOBILE_SCALE : 1;
	#end
	#if mobile
	public static var DEVICE_SCALE = MOBILE_SCALE;
	#end
	 ...
	 */
	public function new(X:Float = 0, Y:Float = 0, ?Text:String, ?OnClick:Void->Void, buttonWidth:Int = 80, buttonHeight:Int = 20, fontSize:Int = 8) {
		super(X, Y, OnClick, Math.floor(buttonWidth * DEVICE_SCALE), Math.floor(buttonHeight * DEVICE_SCALE));
		this.fontSize = Math.floor(fontSize * DEVICE_SCALE);

		initLabel(Text);

		for (point in labelOffsets) {
			point.set(point.x, (this.fontSize / 2) + point.y);
		}
	...
	inline function initLabel(Text:String):Void {
		if (Text != null) {
			label = new FlxText(x + labelOffsets[NORMAL].x, y + labelOffsets[NORMAL].y, buttonWidth, Text, fontSize);
			label.setFormat(null, fontSize, 0xFFFFFF, "center");
...
@:generic
#end
class FlxTypedMetaButton<T:FlxSprite> extends FlxSprite implements IFlxInput {
	/**
	 ...
	 */
	public var labelAlphas:Array<Float> = [0.85, 1.0, 0.5];

	public var labelColors:Array<FlxColor> = [0xFF777777, 0xFF888888, 0xFF777777];

	public var borderColors:Array<FlxColor> = [0xFF222222, 0xFF222222, 0xFF222222];
	 ...
	 * @param   OnClick   The function to call whenever the button is clicked.
	 */
	public function new(X:Float = 0, Y:Float = 0, ?OnClick:Void->Void, buttonWidth:Int = 80, buttonHeight:Int = 20) {
		super(X, Y);
	...
	function updateStatusAnimation():Void {
		makeGraphic(buttonWidth, buttonHeight, labelColors[status]);

		var borderWidth = 2;
		this.drawRect(0, 0, buttonWidth, borderWidth, borderColors[status]); // top
		this.drawRect(0, buttonHeight - borderWidth, buttonWidth, borderWidth, borderColors[status]); // bottom
		this.drawRect(0, 0, borderWidth, buttonHeight, borderColors[status]); // left
		this.drawRect(buttonWidth - borderWidth, 0, borderWidth, buttonHeight, borderColors[status]); // right
	}
	...
	function updateButton():Void {
		...
		if (status != MetalButton.NORMAL && (!overlapFound || (currentInput != null && currentInput.justReleased))) {
			onOutHandler();
		}
	}
	...
	function updateStatus(input:IFlxInput):Void {
		if (input.justPressed) {
			currentInput = input;
			onDownHandler();
		} else if (status == MetalButton.NORMAL) {
	...
	function updateLabelPosition() {
		...
		{
			_spriteLabel.x = (pixelPerfectPosition ? Math.floor(x) : x) + labelOffsets[status].x * MetalButton.DEVICE_SCALE;
			_spriteLabel.y = (pixelPerfectPosition ? Math.floor(y) : y) + labelOffsets[status].y * MetalButton.DEVICE_SCALE;
		}
	}
	...
	function onUpEventListener(_):Void {
		if (visible && exists && active && status == MetalButton.PRESSED) {
			onUpHandler();
		}
	}
	#end
